...
entity daphne_composable_core_top_smoke_tb is
end entity daphne_composable_core_top_smoke_tb;

architecture tb of daphne_composable_core_top_smoke_tb is
  constant AFE_COUNT_C      : positive := 2;
...
begin
  ...
  begin
    ...
    assert trigger_result_s(0) = TRIGGER_XCORR_RESULT_NULL
      report "Disabled self-trigger path should hold the first channel result at the null value"
      severity failure;
    assert descriptor_result_s(CHANNELS_TOTAL_C - 1) = PEAK_DESCRIPTOR_RESULT_NULL
      report "Disabled self-trigger path should hold the last channel descriptor at the null value"
      severity failure;
    assert ready_s(0) = '0'
    ...
    assert false report "daphne_composable_core_top_smoke_tb completed successfully" severity note;
    wait;
  end process;
end architecture tb;
