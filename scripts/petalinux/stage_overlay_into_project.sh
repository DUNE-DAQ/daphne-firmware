...
command -v fdtget >/dev/null 2>&1 || {
  echo "ERROR: fdtget is required to read firmware-name from the DTBO." >&2
  exit 2
}

firmware_name="$(fdtget -t s "$dtbo_src" /amba_pl firmware-name)"
case "$firmware_name" in
  daphne_selftrigger_*.bit.bin) ;;
  *)
    echo "ERROR: unexpected DTBO firmware-name: $firmware_name" >&2
    exit 2
    ;;
esac
...
printf 'git_sha=%s\nfirmware_name=%s\n' \
  "$git_sha" "$firmware_name" > "$STAGED_DIR/BUILD-METADATA.txt"
printf '# Generated by scripts/petalinux/stage_overlay_into_project.sh from the DTBO.\nDAPHNE_OVERLAY_FIRMWARE_NAME = "%s"\n' \
  "$firmware_name" > "$VERSION_INC"
