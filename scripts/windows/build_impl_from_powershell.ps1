...
param(
    ...
    [string]$OutputDir,
    [switch]$StopAfterSynth,
    [switch]$DumpPostSynthDebug
)
...
Set-Or-RemoveEnv -Name DAPHNE_MAX_THREADS -Value ([string]$MaxThreads)
...
Set-Or-RemoveEnv -Name DAPHNE_STOP_AFTER_SYNTH -Value $(if ($StopAfterSynth) { '1' } else { '' })
Set-Or-RemoveEnv -Name DAPHNE_DUMP_POST_SYNTH_DEBUG -Value $(if ($DumpPostSynthDebug -or $StopAfterSynth) { '1' } else { '' })
...
Write-Host "INFO: stop synth  = $($StopAfterSynth.IsPresent)"
Write-Host "INFO: Vivado      = $VivadoBat"

Push-Location $RepoRoot
try {
    & $VivadoBat -mode batch -source (Join-Path $RepoRoot 'xilinx\vivado_impl_entry.tcl')
