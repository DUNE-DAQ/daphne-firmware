...
daphne_board_legacy_manifest_path() {
  root_dir="$1"
  board_name="$2"
  legacy_manifest_rel="$(daphne_board_manifest_value "$root_dir" "$board_name" legacy_manifest)"
  if [ -n "$legacy_manifest_rel" ]; then
    printf '%s/%s' "$root_dir" "$legacy_manifest_rel"
  fi
}
...
daphne_generated_hermes_ip_ready() {
  ...
  for support_path in $(daphne_legacy_support_source_list "$root_dir"); do
    ...
    grep -Fq "$support_leaf" "$component_xml" || return 1
  done
}
...
daphne_board_legacy_manifest_value() {
  root_dir="$1"
  board_name="$2"
  key_name="$3"
  legacy_manifest_path="$(daphne_board_legacy_manifest_path "$root_dir" "$board_name")"

  [ -n "$legacy_manifest_path" ] || return 1
  [ -f "$legacy_manifest_path" ] || return 1

  value="$(daphne_board_manifest_scalar "$legacy_manifest_path" "$key_name")"
  if [ -n "$value" ]; then
    printf '%s' "$value"
    return 0
  fi
...
daphne_board_legacy_manifest_value_with_fallback() {
  root_dir="$1"
  board_name="$2"
  preferred_key="$3"
  fallback_key="$4"

  preferred_value="$(daphne_board_legacy_manifest_value "$root_dir" "$board_name" "$preferred_key")"
  if [ -n "$preferred_value" ]; then
    printf '%s' "$preferred_value"
    return 0
  fi
...
daphne_platform_requires_packaged_ip_preflight() {
  ...
  if daphne_platform_exports_flow_bundle "$root_dir" "$board_name" "$platform_core" "$platform_target"; then
    ...
    if daphne_generated_hermes_ip_ready "$root_dir" "$board_name"; then
      return 1
    fi
...
daphne_resolve_board_defaults() {
  ...
  user_ip_vlnv="$(daphne_board_legacy_manifest_value_with_fallback "$root_dir" "$board_name" legacy_user_ip_vlnv user_ip_vlnv)"
  bd_name="$(daphne_board_legacy_manifest_value_with_fallback "$root_dir" "$board_name" legacy_bd_name bd_name)"
  bd_wrapper_name="$(daphne_board_legacy_manifest_value_with_fallback "$root_dir" "$board_name" legacy_bd_wrapper_name bd_wrapper_name)"
  ...
  ip_cell_name="$(daphne_board_legacy_manifest_value_with_fallback "$root_dir" "$board_name" legacy_ip_cell_name ip_cell_name)"
  ip_component_identifier="$(daphne_board_legacy_manifest_value_with_fallback "$root_dir" "$board_name" legacy_ip_component_identifier ip_component_identifier)"
  ip_display_name="$(daphne_board_legacy_manifest_value_with_fallback "$root_dir" "$board_name" legacy_ip_display_name ip_display_name)"
  ip_xgui_file="$(daphne_board_legacy_manifest_value_with_fallback "$root_dir" "$board_name" legacy_ip_xgui_file ip_xgui_file)"
  ip_cell_bind_root="$(daphne_board_legacy_manifest_value_with_fallback "$root_dir" "$board_name" legacy_ip_cell_bind_root ip_cell_bind_root)"
