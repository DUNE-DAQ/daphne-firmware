...
CORE_PATH="$ROOT_DIR/cores/features/k26c-board-spy-buffer-plane.core"
TOP_PATH="$ROOT_DIR/rtl/isolated/subsystems/spy/k26c_board_spy_buffer_plane.vhd"
...
if [ "$actual_core_deps" != "$expected_core_deps" ]; then
  echo "ERROR: k26c-board-spy-buffer-plane.core direct deps drifted from the spy-buffer contract." >&2
...
if [ "$actual_entity_deps" != "$expected_entity_deps" ]; then
  echo "ERROR: k26c_board_spy_buffer_plane.vhd entity instantiations drifted from the spy-buffer contract." >&2
  echo "INFO: expected:" >&2
...
echo "INFO: Spy-buffer plane remains constrained to the spy boundary and live spy buffer."
