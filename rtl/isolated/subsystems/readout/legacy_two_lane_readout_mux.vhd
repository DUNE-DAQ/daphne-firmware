...
begin
  assert LANE_COUNT_G = 2
    report "legacy_two_lane_readout_mux currently supports exactly two output lanes"
    severity failure;

  assert CHANNEL_COUNT_G = (LANE_COUNT_G * CHANNELS_PER_LANE_G)
    report "legacy_two_lane_readout_mux requires CHANNEL_COUNT_G = LANE_COUNT_G * CHANNELS_PER_LANE_G"
    severity failure;
